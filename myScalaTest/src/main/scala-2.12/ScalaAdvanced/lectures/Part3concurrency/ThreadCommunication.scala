...
/**
  * Created by miguel.ludena on 01/05/2019.
  */
object ThreadCommunication extends App{
  ...
  // WAIT & NOTIFY
  // wait() => an object´s monitor suspends you(the thread) indefinitely
  /*
      //thread 1
      ...
      someObject.synchronized { <- lock the object´s monitor
        ...
        //...code part2 <- when allowed to proceed, lock the monitor again and continue
      }
  ...
  def testNotifyAll() = {
    ...
    new Thread(() => {
      Thread.sleep(2000)
      println(s"[announcer] Rock´nRoll")
      bell.synchronized{
        bell.notifyAll() //Si cambiamos notifyAll por notify, 9 threads quedarían huerfanos no acabarían y la ejecución chunga
      }
    }).start()
  }
  //testNotifyAll()
  ...
  //new Thread(() => sam.bow(pierre)).start() // sam´s lock, then pierre is locked so the can´t execute other.rise
  //new Thread(() => pierre.bow(sam)).start()

  // 3. livelock
  new Thread(() => sam.pass(pierre)).start() // sam´s lock, then pierre is locked so the can´t execute other.rise
