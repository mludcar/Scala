...
/**
  * Created by miguel.ludena on 29/03/2019.
  */
...
/*
  val united = (0 until a.length).map[(Int,Int), IndexedSeq[(Int, Int)]](x => (a(x), b(x))).toList
  println(united.mkString(" - "))

  def solution(a: Array[Int], b: Array[Int]): Int = {
    val data = (0 until a.length).map[(Int,Int), IndexedSeq[(Int, Int)]](x => (a(x), b(x)))

    for{
      i <- 0 until data.length -1
      data(i)._2 != data(i+1)._2

    }yield()

    def meets(first: (Int, Int), second: (Int, Int)): Boolean = {
      if(first._2 == second._2) false
      else if(first._2 == 0 && second._2 == 1) false
      else true
    }
  }
*/
