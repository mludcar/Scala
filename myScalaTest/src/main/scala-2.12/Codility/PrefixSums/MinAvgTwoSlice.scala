...
/**
  * Created by miguel.ludena on 26/03/2019.
  */
/*
A non-empty array A consisting of N integers is given. A pair of integers (P, Q), such that 0 ≤ P < Q < N, is called a
slice of array A (notice that the slice contains at least two elements). The average of a slice (P, Q) is the sum
...
object MinAvgTwoSlice extends App{
  ...
  def solution(a: Array[Int]): Int = {
    ...
    for (i <- (0 until a.length - 1)){
      ...
      if(i < a.length - 2 && ((a(i) + a(i+1) + a(i+2))/3.0) < min_value){
        min_indice = i
