...
object GenomicRangeQuery extends App{
  ...
  //62% Funciona, pero tiene una performance de mierda
  def solutionBad(s: String, p: Array[Int], q: Array[Int]): Array[Int] = {
    def nucleotidToInt(a: Char): Int =
      a match {
        case 'A' => 1
        case 'C' => 2
        case 'G' => 3
    ...
    def searchTuples(): List[(Int, Int)] = (0 until p.length).map{x => if(p(x) <= q(x))(p(x), q(x)) else (q(x), p(x))}.toList

    searchTuples().map{case (from, to) => {
      if(from != to) nucleotidToInt(s.slice(from, to + 1).min)
      else nucleotidToInt(s(from))
    }}.toArray
  }
  ...
  def solution(s: String, p: Array[Int], q: Array[Int]): Array[Int] = {
    ...
    for{
      i <- (0 until p.length).toArray
      (from, to) = (p(i), q(i))
      sequence = s.slice(from, to+1)
    }yield (nucleotidToInt(sequence.min))
  }


  //println(Array(2,1,3,2,2,4).slice(2,4).mkString(" - "))
  println(solution("CAGCCTA", Array(2,5,0), Array(4,5,6)).mkString(" - ")) //[2,4,1]
