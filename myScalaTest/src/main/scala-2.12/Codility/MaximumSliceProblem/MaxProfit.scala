...
days. If a single share was bought on day P and sold on day Q, where 0 ≤ P ≤ Q < N, then the profit of such transaction
...
object MaxProfit extends App{
  ...
  def solution(a: Array[Int]): Int = {
    ...
    val transArray = new Array[Int](a.length)
    transArray(0) = 0
    for(i <- 1 until a.length) transArray(i) = a(i) - a(i-1)

    var newProfit = 0
    var maxProfit = 0

    for(i <- 0 until transArray.length) {
      newProfit = 0 max (newProfit + transArray(i))
      maxProfit = newProfit max maxProfit
    }
    maxProfit
  }
