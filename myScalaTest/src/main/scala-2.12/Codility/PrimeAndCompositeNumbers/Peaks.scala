...
We want to divide this array into blocks containing the same number of elements. More precisely, we want to choose a
number K that will yield the following blocks:
...
(1, 2, 3) blocks do not contain a peak. Notice in particular that the (4, 3, 4) block contains two peaks: A[3] and A[5].
...
object Peaks extends App{
  ...
  def solution(a: Array[Int]): Int = {
    var count = 0
    def comparator(i: Int) = if(a(i) > a(i+1) && a(i) > a(i-1)) true else false
    for(i <- 1 until (a.length - 1)) if(comparator(i)) count += 1
    count
    /*def loop(index: Int, counter: Int): Int = {
      if(index >= a.length - 1) counter
      else if(comparator(index)) loop(index+1, counter+1)
